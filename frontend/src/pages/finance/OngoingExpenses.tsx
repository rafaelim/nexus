...
const OngoingExpenses = () => {
  ...
  const loadExpenses = async () => {
    try {
      setLoading(true);
      const data = await recurringExpenseService.getByType('ongoing', filterActive());
      setExpenses(data);
    } catch (error) {
      console.error('Failed to load ongoing expenses:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <Layout>
      <div class="max-w-7xl mx-auto">
        <div class="flex justify-between items-center mb-6">
          <h1 class="text-3xl font-bold text-gray-900">Ongoing Expenses</h1>
          <button
        ...
        <Show when={loading()} fallback={
          <div class="bg-white shadow rounded-lg overflow-hidden">
            ...
            {expenses().length === 0 && (
              <div class="text-center py-12 text-gray-500">No ongoing expenses found</div>
            )}
          </div>
        }>
